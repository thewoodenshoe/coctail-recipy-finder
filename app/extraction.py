...
RECIPE_LABEL_RE = re.compile(
    r"^(ingredients|to your .* add|to .* add|this one.?s made with .*)\s*:?\s*$",
    re.IGNORECASE,
)
...
def _find_recipe_block(lines: list[str]) -> RecipeBlock | None:
    ...
    for title_index, line in enumerate(lines):
        ...
        title_lines, following_start = _collect_title_lines(lines, title_index)
        display_title = _title_case("  ".join(title_lines))
        following = lines[following_start:]
        ...
        for candidate_index, candidate in enumerate(following):
            ...
            if not in_method and _looks_like_method_start(candidate) and ingredient_lines:
                in_method = True
            if not in_method and ingredient_lines and METHOD_RE.search(candidate) and _looks_like_sentence_context(candidate):
                in_method = True
            if in_method:
            ...
            if (
                ...
                or _looks_like_contextual_ingredient_line(candidate)
                or (ingredient_lines and _looks_like_short_ingredient_continuation(candidate))
                or (
        ...
        return RecipeBlock(
            drink_name=display_title,
            recipe_lines=[display_title, *ingredient_lines, *method_lines],
...
def _collect_title_lines(lines: list[str], title_index: int) -> tuple[list[str], int]:
    title_lines = [_clean_title_candidate(lines[title_index]) or lines[title_index]]
    index = title_index + 1
    while index < len(lines):
        ...
        if not cleaned or not _looks_like_recipe_title(candidate):
            break
        title_lines.append(cleaned)
        index += 1
    return title_lines, index


def _extract_embedded_drink_title(line: str) -> str | None:
    classic = re.search(
        r"\bclassic\s+([A-Za-z][A-Za-z\s]+(?:tini|martini|sour|paloma|margarita|negroni|daiquiri|spritz))\b",
        line,
        re.IGNORECASE,
    )
    if classic:
        return classic.group(1).strip(" .!?").title()
...
def _looks_like_ingredient_line(line: str) -> bool:
    ...
    if re.search(r"(?<![A-Za-z0-9])\d+(-\d+)?\s+(slices?|pods?|cloves?|disks?|strawberries|cucumber slices?)\b", lower):
        return True
    return lower in {"ginger beer", "soda", "club soda", "angostura® aromatic bitters, to top"}
...
def _looks_like_contextual_ingredient_line(line: str) -> bool:
    ...
    if re.search(r"^(juice of|splash of|handful of|lots of|mint for garnish)\b", lower):
        return True
    if re.search(r"^(t|top)\s+with\s+ginger beer\b", lower):
        return True
    if re.search(r"\b(olives|cocktail onions|vermouth|wine|gin|bourbon|tequila|mezcal|rum|vodka|lillet|ginger beer|club soda|crushed ice)\b", lower):
        return len(line) <= 80
    return False


def _looks_like_short_ingredient_continuation(line: str) -> bool:
    if _looks_like_sentence_context(line):
        return False
    if re.search(r"[.!?]", line):
        return False
...
def _looks_like_method_start(line: str) -> bool:
    return bool(
        re.search(
            r"^(combine|in a |toast|add ice|add a few ice|mix|serve|store|top with|muddle|dirty dump|strain|blend|shake|dry shake|stir)",
            line,
            re.IGNORECASE,
        )
    )
...
def _title_case(title: str) -> str:
    ...
    spaced = _collapse_spaced_title(cleaned)
    if spaced:
        return spaced
...
def _collapse_spaced_title(title: str) -> str | None:
    words = []
    for raw_word in re.split(r"\s{2,}", title):
        word = raw_word.strip()
        if not word:
            continue
        if not re.fullmatch(r"(?:[A-Za-z]\s+)+[A-Za-z]", word):
            return None
