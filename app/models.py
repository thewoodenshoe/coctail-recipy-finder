...
class RawPost(Base):
    ...
    raw_thumbnail_url: Mapped[str | None] = mapped_column(Text, nullable=True)
    local_image_path: Mapped[str | None] = mapped_column(Text, nullable=True)
    image_capture_status: Mapped[str | None] = mapped_column(String(64), nullable=True)
    image_capture_error: Mapped[str | None] = mapped_column(Text, nullable=True)
